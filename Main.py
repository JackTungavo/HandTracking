...
class detection():
    ...
    def calibrateSamples(self):
        ...
        by = 0#Biggest ey
        sx = 10000#Smallest ex, arbitarrily large number
        bx = 0#Biggest ex
        for [ex, ey, ew, eh] in self.eyeCoords:
            if ey + eh > by:
                by = ey + eh 
        ...
        for i in range(1, (bx - sx)// 10+1):
            ...
            for y in range (len(upper)):#0<upper<255
                if upper[y] < 0:
                    upper[y] = 0
                if upper [y] > 255:
                    upper[y] = 255
            for y in range (len(lower)):#0<lower<255
                if lower[y] < 0:
                    lower[y] = 0
                if lower [y] > 255:
                    lower[y] = 255
    ...
    def newMoveMouse(self):
        ...
        x = (x/self.cameraWidth)*self.screenWidth#One to one mapping onto the screen resolution
